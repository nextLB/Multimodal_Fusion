...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        self.layer1 = resnet.layer1    # 输出: [batch_size, 256, H/4, W/4]
        self.layer2 = resnet.layer2    # 输出: [batch_size, 512, H/8, W/8]
        self.layer3 = resnet.layer3    # 输出: [batch_size, 1024, H/16, W/16]
        self.layer4 = resnet.layer4    # 输出: [batch_size, 2048, H/32, W/32]
    ...
    def forward(self, x):
        ...
        # 通过四个层级的残差网络提取多尺度特征
        # [batch_size, 64, H/4, W/4] -> [batch_size, 256, H/4, W/4]
        feature1 = self.layer1(x)
        # [batch_size, 256, H/4, W/4] -> [batch_size, 512, H/8, W/8]
        feature2 = self.layer2(feature1)
        # [batch_size, 512, H/8, W/8] -> [batch_size, 1024, H/16, W/16]
        feature3 = self.layer3(feature2)
        # [batch_size, 1024, H/16, W/16] -> [batch_size, 2048, H/32, W/32]
...
class DptHead(nn.Module):
    ...
    def __init__(self, in_channels, features=256, use_bn=False, out_channels=[256, 512, 1024, 2048]):
...
class DepthAnythingResNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        # DPT头部网络用于深度估计
        # ResNet50各层输出通道数: [256, 512, 1024, 2048]
...
def main():
    """主函数：解析参数并启动训练"""
    parser = argparse.ArgumentParser(description='Train Depth Anything with ResNet50 backbone on NYU Depth V2')
